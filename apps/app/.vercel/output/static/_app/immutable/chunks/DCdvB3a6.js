import { n as ge, j as A, q as Oe, t as Pe, u as De, v as Re, k as Le } from './SV1JYOhp.js';
import { d as je, w as re, r as de } from './YlZQZqOj.js';
import {
	a6 as Ke,
	a7 as Me,
	a8 as Be,
...
import { l as W, M as Ye } from './wdwkOUVI.js';
...
function rr(t) {
	...
	return je([e, r, n], ([i, u, a]) => (i || u) && a !== null);
}
function at(t) {
	Oe &&
		ye(1).then(() => {
...
async function or(t) {
	...
	if ((await Promise.all([ye(1), Ye]), e === void 0)) {
		r == null || r.focus();
		return;
	}
	const n = Pe(e) ? e(r) : e;
 ...
 */ var Ne = [
	...
	Ee = function (e, r, n) {
		if (Y(e)) return [];
		var i = Array.prototype.slice.apply(e.querySelectorAll(H));
		return r && P.call(e, H) && i.unshift(e), (i = i.filter(n)), i;
	},
	ke = function t(e, r, n) {
			...
			if (!Y(a, !1))
				if (a.tagName === 'SLOT') {
					...
					var p = a.shadowRoot || (typeof n.getShadowRoot == 'function' && n.getShadowRoot(a)),
						F = !Y(p, !1) && (!n.shadowRootFilter || n.shadowRootFilter(a));
					if (p && F) {
	...
	U = function (e, r) {
		return !(r.disabled || Y(r) || lt(r) || mt(r, e) || dt(r) || gt(r));
	},
	ne = function (e, r) {
...
var pe = {
	...
	Kt = function (e, r) {
		var n = (r == null ? void 0 : r.document) || document,
			i = (r == null ? void 0 : r.trapStack) || jt,
			u = he(
				{
					...
					((a.containerGroups = a.containers.map(function (o) {
						...
						return {
							...
							nextTabbableNode: function (C) {
								...
								return I < 0
									? M
										? l.slice(l.indexOf(C) + 1).find(function (B) {
												return L(B);
											})
										: l
												.slice(0, l.indexOf(C))
					...
					if (v < 0)
						...
						if (
							...
							var C = T === 0 ? a.tabbableGroups.length - 1 : T - 1,
								M = a.tabbableGroups[C];
							g = O(s) >= 0 ? M.lastTabbableNode : M.lastDomTabbableNode;
						...
						if (
							...
							var B = I === a.tabbableGroups.length - 1 ? 0 : I + 1,
								le = a.tabbableGroups[B];
			...
			D = function (o) {
				var s = V(o);
				if (!(f(s, o) >= 0)) {
					if (q(u.clickOutsideDeactivates, o)) {
						d.deactivate({ returnFocus: u.returnFocusOnDeactivate });
						return;
					}
					q(u.allowOutsideClick, o) || o.preventDefault();
				}
			},
			R = function (o) {
				var s = V(o),
			...
			oe = function (o) {
				Dt(o) && q(u.escapeDeactivates, o) !== !1 && (o.preventDefault(), d.deactivate());
			},
			se = function (o) {
				var s = V(o);
				f(s, o) >= 0 ||
					q(u.clickOutsideDeactivates, o) ||
					q(u.allowOutsideClick, o) ||
				...
				_setPausedState: {
					value: function (o, s) {
						...
						if (((a.paused = o), o)) {
							var l = c(s, 'onPause'),
								b = c(s, 'onPostPause');
							l == null || l(), ce(), K(), b == null || b();
						} else {
							var h = c(s, 'onUnpause'),
								g = c(s, 'onPostUnpause');
							h == null || h(), F(), ue(), K(), g == null || g();
						}
						return this;
					}
				}
...
const Bt = { floating: {}, focusTrap: {}, clickOutside: {}, escapeKeydown: {}, portal: 'body' },
	sr = (t, e) => {
		t.dataset.escapee = '';
		...
		const u = { ...Bt, ...i },
			a = [];
		if (u.portal !== null) {
			const c = Me(t, u.portal);
			c != null && c.destroy && a.push(c.destroy);
		}
		if ((a.push(Be(r, t, u.floating).destroy), u.focusTrap !== null)) {
			const { useFocusTrap: c } = Mt({
					immediate: !0,
					escapeDeactivates: !1,
					allowOutsideClick: !0,
	...
	qt = { orientation: 'horizontal', decorative: !1 },
	ur = (t) => {
		const e = { ...qt, ...t },
			r = qe(e),
	...
	cr = async (t, e = 'name', r = !1) => {
		if (((t = t || W($)), !t)) throw new Error('No directory path provided');
		if (
			(
				await j
					.select()
		...
		const i = await j
				.select()
				.from(Q)
				.where(We(_(Q.collectionPath, W($)), t !== W($) ? _(Q.parentPath, t) : void 0)),
			u = _e(i, t),
			a = (c) => {
				c.sort((f, y) =>
		...
		return Ge.set(r ? u : d(u)), r ? u : d(u);
	},
	lr = async (t) => {
		if (!t) return;
		$.set(t), He.set([]), Xe.set(null);
		const e = { path: t, name: t.split('/').pop(), lastOpened: new Date() },
			r = await j.select().from(x).where(_(x.path, t));
		r && r.length > 0
			? await j.update(x).set({ lastOpened: new Date() }).where(_(x.path, t))
			: await j.insert(x).values(e);
	},
	dr = async () => await j.select().from(x);
export {
	ur as a,
	tr as b,
	ir as c,
